...
input.forEach(function(present) {
  ...
  // For part 2
  var highest = parseInt(Math.max.apply(null, measurements));
  ...
  var bowRibbon = measurements[0] * measurements[1] * measurements[2];


  //measurements.splice(measurements.indexOf(highest.toString()));
  var wrapRibbon = (measurements[0] * 2) + (measurements[1] * 2);


  //measurements.map(function(item) {
  //  wrapRibbon += (parseInt(item) + parseInt(item))
  //});
  ...
  totalAmount += total;
  totalRibbon += (parseInt(wrapRibbon) + parseInt(bowRibbon))
})
